...
export default function JobCard({
  ...
  const [analysis, setAnalysis] = useState(
    job.aiAnalysis ? JSON.parse(job.aiAnalysis) : null,
  );
  ...
  const userSkills =
    job.extractedSkills ||
    skills ||
    "Java, Spring Boot, React.js, Node.js, MongoDB, REST APIs";
  ...
  const handleAnalyze = async () => {
    if (onAIAction && !isStatic) {
      onAIAction("analysis");
      return;
    }
    if (!job.jobDescription)
      return toast.error("No job description to analyze");
    setLoading(true);
    setTab("analysis");
    try {
    ...
    return (
      ...
      >
            ...
            <Section title="✅ Strong Matches">
              <div style={styles.tags}>
                {analysis.strongMatches?.map((s) => (
                  <span key={s} style={styles.greenTag}>
                    {s}
                  </span>
                ))}
              </div>
            ...
            <Section title="⚠️ Skill Gaps">
              <div style={styles.tags}>
                {analysis.skillGaps?.map((s) => (
                  <span key={s} style={styles.redTag}>
                    {s}
                  </span>
                ))}
              </div>
        ...
        {tab === "interview" && interviewQ && (
          <div>
            <Section title="💻 Technical Questions">
              {interviewQ.technical?.map((q, i) => (
                <p key={i} style={styles.question}>
                  Q{i + 1}. {q}
                </p>
              ))}
            </Section>
            <Section title="🤝 Behavioral Questions">
              {interviewQ.behavioral?.map((q, i) => (
                <p key={i} style={styles.question}>
                  Q{i + 1}. {q}
                </p>
              ))}
        ...
        {tab === "analysis" && !analysis && !loading && (
          <button style={styles.saveBtn} onClick={handleAnalyze}>
            Generate Analysis
          </button>
        )}
        {tab === "cover" && !coverLetter && !loading && (
          <button style={styles.saveBtn} onClick={handleCoverLetter}>
            Generate Cover Letter
          </button>
        )}
        {tab === "interview" && !interviewQ && !loading && (
          <button style={styles.saveBtn} onClick={handleInterviewPrep}>
  ...
  return (
    ...
    >
      ...
      {job.resumeFileName && (
        <p style={styles.resumeInfo}>
          📁 Resume:{" "}
          <span style={{ color: "#34d399" }}>{job.resumeFileName}</span>
        </p>
      )}
      {job.resumeSummary && (
        <p style={styles.resumeSummaryText}>💼 {job.resumeSummary}</p>
      )}
      {isOverdue && <p style={styles.overdue}>⚠️ Follow-up overdue!</p>}
      {job.followUpDate && !isOverdue && (
      ...
      <div style={styles.actions}>
        <button style={styles.aiBtn} onClick={handleAnalyze} disabled={loading}>
          🤖 Analyze
        </button>
        <button
          style={styles.aiBtn}
...
const styles = {
  ...
  resumeInfo: { fontSize: '13px', color: '#64748b', marginBottom: '4px' },
  resumeSummaryText: { fontSize: '13px', color: '#94a3b8', marginBottom: '8px', fontStyle: 'italic', lineHeight: '1.5' },
  badge: {
    padding: "8px 20px",
    borderRadius: "30px",
    fontSize: "16px",
